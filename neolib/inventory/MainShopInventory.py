...
class MainShopInventory(Inventory):
    ...
    def _loadItems(self, usr, shopID):
        ...
        try:
            if not pg.find("p", "soldOutMessage"):
                for row in pg.find("div", "shop-grid").find_all("tr"):
                    for item in row.find_all("div", "shop-item"):
                        tmpItem = MainShopItem(item.b.text)

                        tmpItem.id, tmpItem.stockid, tmpItem.brr = re.findall("j_info_id=(.*?)&stock_id=(.*?)&g=(.*?)", item.div['data-link'])[0]
